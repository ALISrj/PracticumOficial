...
object SentenciasSQL {
  ...
  def principal2() =
    ...
    val estadios: List[Estadio] = estadiosXCapacidad(10000)
      .transact(xa)
      .unsafeRunSync()
  ...
  def hostMundial(id:String): ConnectionIO[List[Host]] =
    sql"""
         SELECT h.countryId, c.name
         FROM hostcountries h
         INNER JOIN countries c ON h.countryId = c.countryId
  ...
  def ganadores(): ConnectionIO[List[Winner]] =
    sql"""
         SELECT c.countryId, c.name, t.name
         FROM countries c
         INNER JOIN tournaments t ON c.countryId = t.countryIdWinner
         """
      .query[Winner]
      .to[List]
  ...
  def alineacionesDJugador(nombre:String): ConnectionIO[List[Alineacion]] =
    sql"""
    SELECT s.tournamentId, c.name, s.positionName
    FROM squads s
  ...
  def estadiosXCapacidad(capacidad:Int): ConnectionIO[List[Estadio]] =
    sql"""
         SELECT s.name, s.cityName, c.name, s.capacity
         FROM stadiums s
         INNER JOIN countries c ON s.countryId = c.countryId
         WHERE s.capacity = $capacidad
         """
      .query[Estadio]
      .to[List]
  ...
  def fechaNacimiento(fecha:String): ConnectionIO[List[Jugador]] =
    sql"""
         SELECT p.familyName, p.givenName, p.birthdate
         FROM players p
         WHERE DATE_FORMAT(p.birthdate, '%m-%d') = $fecha
         """
      .query[Jugador]
      .to[List]
}
